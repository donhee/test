...
public class TaskController {
    ...
    public TaskController(Scanner scanner, TeamDao teamDao, TaskDao taskDao,MemberDao memberDao,TeamMemberDao teamMemberDao) {
    ...
    private void onTaskUpdate(final Team team) {
        ...
        str = keyScan.nextLine();
        if (str.length() == 0) {
            task.setStartDate(originTask.getEndDate());
        } else {
            Date date = Date.valueOf(str);
            if (date.getTime() > team.getEndDate().getTime()) {
                task.setEndDate(team.getEndDate());
    ...
    private void onTaskState(final Team team) {
        ...
        if (state == Task.READY || state == Task.WORKING || state == Task.COMPLETE) {
            ...
            System.out.println("올바르지 못한 값입니다. 이전 상태를 유지합니다.");
        }
